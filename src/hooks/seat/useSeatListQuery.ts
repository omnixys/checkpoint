...
export default function useSeatListQuery({
  eventId,
  loadSeatList,
}: Props) {
  const getSeatLisQueryResult = useQuery<SeatListQuery, SeatListQueryVariables>(
    SeatListDocument,
    {
  ...
  const [getSeatList] = useLazyQuery<SeatListQuery, SeatListQueryVariables>(
    SeatListDocument,
    {
      fetchPolicy: "cache-and-network",
    },
