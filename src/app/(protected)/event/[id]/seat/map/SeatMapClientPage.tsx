...
export default function SeatMapClientPage() {
  ...
  const { data: mapData, loading: mapLoading, refetch: refetchMap } = useQuery<SeatMapViewQuery>(
    SeatMapViewDocument,
    {
      variables: { eventId },
  ...
  const presenceMap = React.useMemo(() => {
    ...
    if (!mapData?.seatPresencesByEvent) return map;
  ...
  const highlightedSeatIds = React.useMemo(() => {
    ...
    if (!hasSearch && !hasPresenceFilter) return undefined;
  ...
  const ownSeatIds = React.useMemo(() => {
    if (!currentUser?.id || !seatList) return new Set<string>();
    return new Set(
      seatList
  ...
  const handleDuplicateTable = async () => {
    ...
    if (!table || selectedItems.length !== 1) return;
    try {
  ...
  const handleCloneSection = async () => {
    ...
    if (!section || selectedItems.length !== 1) return;
    try {
      await cloneSection({
        variables: {
      ...
      console.error("Failed to clone section", err);
    }
  };
  ...
  const handleMoveSeat = async (seatId: string, x: number, y: number) => {
    try {
      ...
      console.error("Move seat failed", err);
    }
  };
  ...
  const handleRename = async (updates: { id: string; type: SelectedItem["type"]; newName: string }[]) => {
    try {
